...
output_dir = "./output" 
# e.g. output_dir = "C:/some_folder/output/"
...
sample_dir = "C:/Users/Cupcake/Documents/BDAtest1/input/waterfall plot"
input_dir= "C:/Users/Cupcake/Documents/BDAtest1/input"
#, system.file() accesses the example data coming with the package!
...
absorbance_dir = "C:/Users/Cupcake/Documents/BDAtest1/input/absorbance"
#package = "staRdom" 
# e.g. absorbance_dir = "C:/some_folder/input/absorbance/", system.file() accesses the exmaple data coming with the package!
...
absorbance_path = 10 # e.g. absorbance_path = 5 #changed from 1 to 10
...
metadata = "C:/Users/Cupcake/Documents/BDAtest1/META_200625.csv"
# e.g. metadata = "C:/some_folder/input/metatable.csv", system.file() accesses the exmaple data coming with the package!
...
abs_norm = TRUE # e.g. TRUE, c(700,800) #changed from FALSE
...
# wanted it to be!
dil_sample_name_correction = FALSE #changed to false due to error in dilution dimensions
...
# Emission wavelength:
em_range = c(250,800) # e.g. c(300,500), c(0,Inf) to use everything #changed from (0,Inf) to (250,800)

# Excitation wavelength:
ex_range = c(250,800) # e.g. c(300,500), c(0,Inf) to use everything #same as above

# Cut all samples to fit largest range available in all samples
cut_range_to_smallest = TRUE # e.g. FALSE #set to TRUE
...
blank_correction = TRUE # e.g. FALSE #set to TRUE
...
ife_correction = TRUE # e.g. FALSE # I changed it to true, then back to false, like, a dozen times
...
remove_scatter_width = c(15, 15, 20, 20) # e.g. 15 or c(15,15,15,15) #originally c(15,20,15,18)

# state whether removed scattering should be interpolated
interpolation <- FALSE # e.g. TRUE #set to FALSE
...
raman_normalisation = "meta" # e.g. "blank", FALSE, 160, "meta" # changed from FALSE to "blank" to "meta"
...
if(absorbance_dir != FALSE){
  abs_data <- absorbance_read(absorbance_dir, verbose = TRUE, cores = 2)
  if(abs_norm != FALSE){
    if(abs_norm == TRUE) abs_norm <- c(240, 450) # CHANGED C(240, 700) TO C(240,450)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
if(cut_range_to_smallest) eem_list <- eem_list %>% eem_red2smallest() #Gave error, commenting out
...
if(data_file != FALSE){
  save(list=c(eem_name,"abs_data"),file = paste0(output_dir,data_file,"_",format(Sys.time(), "%Y%m%d_%H%M%S"),".RData")) # Was "list=c(eem_name,"abs_data","meta")", but no meta so removed meta
}
```
# Sample overview
